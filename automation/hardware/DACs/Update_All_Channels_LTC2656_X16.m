function [errors]=Update_All_Channels_LTC2656_X16(Master_Object,BASE_ADDRESS)
...
if status_register(6) & status_register(7)
    Master_Object.write('uint32',sprintf('0x%08X',hex2dec(BASE_ADDRESS)+4),hex2dec('001F0000'));
else
...
%Power Down
if status_register(6) & status_register(7)
    Master_Object.write('uint32',sprintf('0x%08X',hex2dec(BASE_ADDRESS)+4),hex2dec('004F0000'));
else
   while ~(status_register(6) & status_register(7))
       %Check the status of the interface
        status_register=Master_Object.read('uint32',sprintf('0x%08X',hex2dec(BASE_ADDRESS)+8),1);
